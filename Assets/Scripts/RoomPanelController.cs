...
public class RoomPanelController : PunBehaviour {
	...
	void OnEnable () {
		...
		for (int i = 0; i < teamSize; i++) {	
			if (!Team1 [i].activeSelf) {		//在队伍1找到空余位置
				...
				else texts [1].text = "Unready";							//如果玩家不是MasterClient，玩家状态显示"未准备"
	...
	public void ClickSwitchButton(){
		...
		if (!isSwitched)
            ...
            promptMessage.text = "cannot change team, the other team is already full";   //如果玩家切换队伍失败，提示信息显示"另一队伍已满，无法切换"
		else
			promptMessage.text = "";
	}
	...
	public void ClickStartGameButton(){
		foreach (PhotonPlayer p in PhotonNetwork.playerList) {		//遍历房间内所有玩家
			...
			if ((bool)p.customProperties ["isReady"] == false) {	//如果有人未准备
                ...
                promptMessage.text = "Not all the payers are ready";      //提示信息显示"有人未准备，游戏无法开始"
				return;												//结束函数执行
			}
		}
